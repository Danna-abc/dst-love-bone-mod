...
-- 1. 标准创建动作，不接收返回值
AddAction("USE_WANGSHENG_BONE", "Use Wangsheng Bone", function(act)
...
AddAction("USE_BENFU_BONE", "Use Benfu Bone", function(act)
    if not TheWorld.ismastersim then return true end
    local user = act.doer
    local bone = act.invobject
...
-- 2. 标准状态机绑定，直接从GLOBAL.ACTIONS取动作
...
-- 3. 背包道具右键标准挂载方式（替换老旧PostInit添加Action）

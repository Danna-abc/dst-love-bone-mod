...
ACT_WANGSHENG.id = "USE_WANGSHENG_BONE"
ACT_WANGSHENG.str = "Use Wangsheng Bone"
ACT_WANGSHENG.fn = function(act)
    if not TheNet:GetIsServer() then return true end
    local user = act.doer
...
if TheNet:GetIsServer() then
    GLOBAL.AddPrefabPostInit("wangsheng_bone", function(inst)
    ...
    GLOBAL.AddPrefabPostInit("benfu_bone", function(inst)
        inst:DoTaskInTime(0.01, function()
            if inst.components.inventoryitem then
